    ...
    public void run() {//这里线程处于run状态，可以发送/接受消息
        while (true) {
            try {
                ...
                System.out.println("サーバーとクライアント" + userId + "は常に通信を保ち、データを読み取ります。..");
                ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
                Message message = (Message) ois.readObject();
                //根据message的类型，做相应的业务处理
                if(message.getMesType().equals(MessageType.MESSAGE_GET_ONLINE_FRIEND)) {
                    //客户端要在线用户列表
                    //System.out.println(message.getSender() + "要在线用户列表");//オンラインユーザーリスト
                    System.out.println(message.getSender() + "オンラインユーザーリスト");//オンラインユーザーリスト
                    ...
                    System.out.println("其他类型message暂不 处理");
                }
